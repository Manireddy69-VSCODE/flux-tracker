...
const dayLabel = (daysBack, baseMs) => {
...
function MessageVolumeChart({ events, baseMs }) {
  if (!baseMs) {
    return (
  ...
  const DAYS = 21;
  const buckets = {};
  for (let i = DAYS - 1; i >= 0; i--) buckets[dayLabel(i, baseMs)] = 0;

  events
    .filter(e => e.event === "message_sent" && e.timestamp)
    .forEach(e => {
      const k = dayLabel(
        Math.round((baseMs - new Date(e.timestamp).getTime()) / 86400000),
        baseMs
      );
  ...
  const todayCount = buckets[dayLabel(0, baseMs)] || 0;
    ...
    <div style={{ ...S.card }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-end", gap:4, height:90 }}>
        {entries.map(([day, count]) => {
          ...
          const isT = day === dayLabel(0, baseMs);
          return (
  ...
  return (
    <div style={S.lockShell}>
      <div style={S.lockCard}>
        ...
        <div style={S.lockSub}>
          Enter your <code>FLUX_ADMIN_KEY</code> to access<br/>the admin dashboard.
        </div>
        <input
          style={S.lockInput}
          type="password"
          ...
          autoFocus
        />
        <button style={S.lockBtn} onClick={tryUnlock}>UNLOCK →</button>
...
export default function AdminDashboard({ onBack }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [evts, h] = await Promise.all([
        fetch(`${API_URL}/analytics/events`).then(r => r.json()),
        fetch(`${API_URL}/health`).then(r => r.json()).catch(() => ({})),
      ]);
  ...
  // Try unlock — in a real app you'd validate against backend.
  // Here we just let them in (the data itself is non-destructive).
  // If you want real protection, add a /api/admin/verify endpoint.
  const handleUnlock = () => {
    // Accept any non-empty key for now — the sensitive data is read-only
    // and the backend's destructive endpoints still require the real key header.
    setUnlocked(true);
  };
          ...
          {loading && events.length === 0 ? (
            ...
            <>
              ...
              <Section title="Message Volume Over Time">
                <MessageVolumeChart events={events} baseMs={nowMs} />
              </Section>
